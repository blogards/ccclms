...
function login(){
	...
	if (count($errors) == 0) {
		...
		$query = "SELECT users.id, email, user_type, first_name, middle_name, last_name, profile_img, login_status 
				FROM users, borrowers 
				WHERE email='$email' AND password='$password' AND users.id = borrowers.id LIMIT 1";
...
function register(){
	...
	if (count($errors) == 0) {
		...
		if (isset($_POST['user_type'])) {
			...
			$query = "INSERT INTO users (email, password, user_type, status, profile_img) 
					  VALUES('$email', '$password', '$user_type', '$status', '$profilepic')";

			$query .= "INSERT INTO borrowers (first_name, middle_name, last_name, contact) 
					  VALUES('$firstname', '$middlename', '$lastname', '$contact')";
			...
			$query = "INSERT INTO users (email, password, user_type, status, profile_img) 
					  VALUES('$email', '$password', 'user', '$status', '$profilepic');";
